...
Int_t n_par = 20;
Double_t m_parameters[20]; 
Double_t m_parerror[20];
...
void  SetParameters(double *par)
{
    if(option == 1) {
        ...
        fAbs->SetParameter(0, par[3]);
        fAbs->SetParameter(1, par[4]);
        fAbs->SetParameter(2, par[5]);
        fAbs->SetParameter(3, par[6]);
        fAbs->SetParameter(4, par[7]);
        nu_lambda = par[8];
        nu_R = par[9];
        nu_f = par[10];
        fRayLength->SetParameter(1, par[11]);
        if (constrain == 0)
...
void DrawTrans()
{
    ...
    for (Int_t i=0; i<1000; i++) {
        ...
        if (option == 0)
            ...
            rindex = fRindex->Eval(wl);
        fRayLength->SetParameter(0, rindex);
        fRayLength->SetParameter(1, m_parameters[11]);
        fCorr->SetParameter(0, rindex);
        Double_t corr = fCorr->Eval(wl);
        Double_t rayL = fRayLength->Eval(wl);
        Double_t T_abs = fAbs->Eval(wl);
